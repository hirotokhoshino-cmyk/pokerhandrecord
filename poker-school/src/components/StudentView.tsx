...
export function StudentView({ user, scores }: Props) {
  ...
  const totalBalance = myScores.reduce((s, x) => s + (x.balance ?? 0), 0);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-[#1a1d27] rounded-xl p-3 border border-slate-800 text-center">
          <p className="text-xs text-slate-500">総合収支</p>
          <p className={`text-2xl font-bold mt-1 ${totalBalance >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
