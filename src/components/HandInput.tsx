...
interface Props {
  ...
  currentStack: number;  // $ for pre-filling effective stack
  defaultBbSize: number;
}

export function HandInput({ onAdd, currentStack, defaultBbSize }: Props) {
  ...
  const submitAmount = (e: React.FormEvent) => {
    ...
    if (isNaN(val) || raw.trim() === '') { setError('数値を入力してください'); return; }
    setPending(val);
    setError('');
    setStep('history');
  };
  ...
  const saveWithHistory = (history: HandHistory) => {
    onAdd(pending, note.trim() || undefined, history);
    reset();
  };

  const saveWithout = () => {
    onAdd(pending, note.trim() || undefined);
    reset();
  };

  const reset = () => { setRaw(''); setNote(''); setPending(0); setStep('amount'); };

  if (step === 'history') {
    return (
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className={`text-lg font-bold font-mono ${pending >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {pending >= 0 ? '+' : ''}${pending.toLocaleString()}
          </span>
          <button type="button" onClick={reset} className="text-xs text-slate-500 hover:text-slate-300">← 戻る</button>
        </div>
        <HandHistoryForm
          onSave={saveWithHistory}
          onCancel={saveWithout}
          initialStackDollars={currentStack}
          defaultBbSize={defaultBbSize}
        />
      </div>
    );
  }
